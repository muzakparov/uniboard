...
(function($) {
    ...
    // Hide taxonomy tags in the Calendar, and only show first two
    // and add a "more" link to show the rest if the user wants to see them

    Drupal.behaviors.cb_psatTaxonomyTags = {

        attach: function() {
            var anchorCollection = $(".psatCalendarView .view-content .item-list, .psatResourcesView .view-content .item-list");
            var ulTempHolder, ulLiTempHolder, ulTempHolderChosen;

            if($(".psatCalendarView").length > 0 || $(".psatResourcesView").length > 0){ // Make sure we are on the right page first

                anchorCollection.each(function() { // Loop through each item-list in the view table
                    ulTempHolder = $(this).find("ul");
                    ulLiTempHolder = ulTempHolder.find("li");
                    ulTempHolderChosen = ulTempHolder.find("li:first-child, li:first-child+li"); // First two elements

                    // If there are 3 terms, just show the terms. If there are 4 or more, show only the first 2 + the more button.
                    if(ulLiTempHolder.length > 3){
                        ulLiTempHolder.hide();
                        ulTempHolderChosen.show();
                        ulTempHolder.append("<li class=\"cbPsatTaxonomyMore\"><a class=\"cbMoreLink\" href=\"#\"><span class=\"sr-only\">Show </span>more<span class=\"sr-only\">tags by clicking this link.</span></a></li>");
                    } // END IF ulLiTempHolder.length > 3
                }); // END EACH


                $(".cbPsatTaxonomyMore .cbMoreLink").on('keypress click', function(e) {
                    // Check if they clicked enter, space, or mouse clicked - Accessibility
                        if ($(window).keyTrigger(e)){
                            e.preventDefault();
                            // Show all the tags if available
                            $(this).parent().parent().find("li").show();

                            // Focus on the first hidden tag, so the keyboard doesn't jump back to the top - Accessibility
                            $(this).parent().parent().find("li:first-child+li+li a").focus();

                            $(this).parent().remove(); // Hide the More link if they click on this

                        }
                });
            } //END .psatCalendarView").length > 0
        } // END ATTACH
